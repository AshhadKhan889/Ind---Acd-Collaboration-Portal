...
// ✅ Academia Dashboard → Posted Opportunities + Upcoming Deadlines
router.get("/dashboard", protect, isAcademia, async (req, res) => {
  try {
    const academiaId = req.user.id;

    // ✅ Fetch all opportunities posted by this academia user
    ...
    // ✅ Upcoming Deadlines across all types (next 5)
