  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        ...
        const res = await axios.get('http://localhost:5000/api/industry/dashboard', {
        ...
        setError(
          err?.response?.data?.message ||
            (err?.response?.status === 403
              ? 'Access denied: Industry Official role required'
                ...
                {data.upcomingDeadlines.jobs.length > 0 ? (
                  data.upcomingDeadlines.jobs.map((job) => (
                    <ListItem key={job._id}>
                      <ListItemText
                        primary={job.jobTitle}
                        secondary={`Deadline: ${new Date(job.applicationDeadline).toLocaleDateString()}`}
                      />
                    </ListItem>
                  ))
                ) : (
                  <Typography color="text.secondary">No upcoming job deadlines</Typography>
                )}
              </List>
                ...
                {data.upcomingDeadlines.projects.length > 0 ? (
                  data.upcomingDeadlines.projects.map((proj) => (
                    <ListItem key={proj._id}>
                      <ListItemText
                        primary={proj.projectTitle}
                        secondary={`Deadline: ${new Date(proj.timeline?.applicationDeadline).toLocaleDateString()}`}
                      />
                    </ListItem>
                  ))
                ) : (
                  <Typography color="text.secondary">No upcoming project deadlines</Typography>
                )}
              </List>
                ...
                {data.upcomingDeadlines.internships.length > 0 ? (
                  data.upcomingDeadlines.internships.map((intern) => (
                    <ListItem key={intern._id}>
                      <ListItemText
                        primary={intern.title}
                        secondary={`Deadline: ${new Date(intern.applicationDeadline).toLocaleDateString()}`}
                      />
                    </ListItem>
                  ))
                ) : (
                  <Typography color="text.secondary">No upcoming internship deadlines</Typography>
                )}
              </List>
            </CardContent>
          </Card>
